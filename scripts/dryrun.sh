...
LEGACY_FIXTURE="$REPO_ROOT/test-keys/key-scrypt.json"
...
write_config() {
  cat > "$HOT_CHEESE_HOME/config.toml" <<TOML
service = "$1"
account = "$2"
store = "$CFG_STORE"
port = $DRY_PORT
...
on_exit() {
  ...
  echo "  note  dry-run home (deleted only by phase 7): ${HOT_CHEESE_HOME:-unset}"
  exit $rc
}
...
echo "  phases 0-7 total: 8 Touch ID sheets + 6 recovery-passphrase entries"
echo "    phase 0  2 Touch ID   (se-selftest: two enclave ECDHs)"
echo "    phase 1  2 passphrase (init: new passphrase, entered twice)"
echo "    phase 2  3 passphrase + 1 Touch ID"
echo "    phase 3  3 Touch ID   (plus one short secret you type at a hidden prompt)"
echo "    phase 4  1 Touch ID   (the allowed sign; all five policy refusals cost 0)"
echo "    phase 5  1 Touch ID   (one live /read through the daemon)"
echo "    phase 6  1 passphrase (the recovery escape hatch, SE blob moved aside)"
echo "    phase 7  0"
...
echo
echo "==> required tools"
for tool in cargo swiftc openssl curl shasum lsof security cmp stat find tr; do
...
if [[ "$BIN" -nt "$REPO_ROOT/swift/se_bridge.swift" ]]; then
  pass "binary is newer than swift/se_bridge.swift"
else
  fail "binary is OLDER than swift/se_bridge.swift — you are about to test a stale enclave bridge"
fi
if [[ "$BIN" -nt "$REPO_ROOT/src/mac/secure_enclave.rs" ]]; then
  pass "binary is newer than src/mac/secure_enclave.rs"
else
  fail "binary is OLDER than src/mac/secure_enclave.rs — you are about to test a stale enclave path"
fi

if [[ -n "$SKIP_SELFTEST" ]]; then
  echo
  echo "==> SKIP_SELFTEST is set: skipping se-selftest (saves 2 Touch ID prompts)"
else
  echo
  echo "==> se-selftest on real Secure Enclave hardware"
  echo "    Expect TWO Touch ID sheets. It creates a throwaway enclave key, proves the ECDH is"
  echo "    deterministic AND equals the host-side ECDH used at enrollment, then deletes the key."
  TAPS_EXPECTED=$((TAPS_EXPECTED + 2))
  BIO_START="$(date '+%Y-%m-%d %H:%M:%S')"
  ...
  assert_taps "se-selftest cost exactly two Touch ID sheets" "2" "$BIO_START"
  if [[ -e "$HOT_CHEESE_HOME/$SELFTEST_BLOB_NAME" ]]; then
    fail "se-selftest left its throwaway key blob behind: $HOT_CHEESE_HOME/$SELFTEST_BLOB_NAME"
  else
    pass "se-selftest left no throwaway key blob"
  fi
  LEFTOVER="$(find "$HOT_CHEESE_HOME" -maxdepth 1 -name 'se_kek_*.blob' 2>/dev/null || true)"
  assert_eq "no SE blob of any label remains after se-selftest" "" "$LEFTOVER"
fi
...
echo "    (CN=localhost, SAN DNS:localhost + IP:127.0.0.1, EKU serverAuth) so phase 5 serves"
...
if [[ -e "$HOT_CHEESE_HOME/config.toml" ]]; then
...
if [[ -f "$HOT_CHEESE_HOME/config.toml" ]]; then
...
CFG_SERVICE="$(grep -E '^[[:space:]]*service[[:space:]]*=' "$HOT_CHEESE_HOME/config.toml" | head -1 | cut -d'"' -f2 || true)"
CFG_ACCOUNT="$(grep -E '^[[:space:]]*account[[:space:]]*=' "$HOT_CHEESE_HOME/config.toml" | head -1 | cut -d'"' -f2 || true)"
CFG_STORE="$(grep -E '^[[:space:]]*store[[:space:]]*=' "$HOT_CHEESE_HOME/config.toml" | head -1 | cut -d'"' -f2 || true)"
if [[ -n "$CFG_SERVICE" && -n "$CFG_ACCOUNT" && -n "$CFG_STORE" ]]; then
  pass "read service/account/store back out of the generated config.toml"
else
  fail "could not read service/account/store out of $HOT_CHEESE_HOME/config.toml"
fi
...
REAL_BLOBS="$(find "$REAL_HOME" -maxdepth 1 -name 'se_kek_*.blob' 2>/dev/null || true)"
assert_eq "no SE blob appeared in the real home" "" "$REAL_BLOBS"
...
phase "PHASE 3 — key lifecycle on the real Secure Enclave (3 Touch ID)"

echo "Every command below unlocks the DEK through the enclave. One sheet each."
...
phase "PHASE 4 — scoped signing (1 Touch ID; every denial costs 0)"

echo "Policy load and evaluation both run BEFORE the approval sheet, so the five refusals"
echo "below cost no biometric at all."
...
echo
echo "==> 4a: no policy file at all. Deny by default: a perfectly valid intent must still fail."
BIO_PHASE="$(date '+%Y-%m-%d %H:%M:%S')"
if [[ -e "$POLICY_FILE" ]]; then
  fail "a policy file already exists at $POLICY_FILE"
fi
assert_cmd_fails_with p4a_no_policy "Policy(Io(" "$BIN" sign --file "$INTENT_OK"
assert_not_contains "4a showed no approval banner" "policy: ALLOWED" "$RUN_LOG"

echo
echo "==> 4b: a policy WITHOUT chain_id must fail to LOAD. This is exactly what a stale"
...
operation = "call"
TOML
assert_cmd_fails_with p4b_no_chain_id "Policy(Toml(" "$BIN" sign --file "$INTENT_OK"
assert_not_contains "4b showed no approval banner" "policy: ALLOWED" "$RUN_LOG"

echo
echo "==> 4c: the good policy, then the allowed transfer."
cat > "$POLICY_FILE" <<TOML
safe = "$SAFE_ADDR"
chain_id = 1
...
echo "    the approval biometric is reused for the key unlock. Only {r,s,v} comes back."
TAPS_EXPECTED=$((TAPS_EXPECTED + 1))
BIO_START="$(date '+%Y-%m-%d %H:%M:%S')"
run_tty p4c_sign_ok "$BIN" sign --file "$INTENT_OK"
...
assert_taps "the pre-evaluated LAContext was reused: one sign, one sheet (2 means the reuse broke)" \
  "1" "$BIO_START"

echo
echo "==> 4d: the gas-refund DRAIN. Allowed 'to', allowed selector, value 0 — and it still"
echo "    drains the Safe through the refund fields. Fail-closed without a [refunds] opt-in."
assert_cmd_fails_with p4d_drain "RefundNotAllowed" "$BIN" sign --file "$INTENT_DRAIN"
assert_not_contains "4d showed no approval banner" "policy: ALLOWED" "$RUN_LOG"

echo
echo "==> 4e: a destination outside the allow-list."
assert_cmd_fails_with p4e_bad_to "ToNotAllowed" "$BIN" sign --file "$INTENT_BAD_TO"
assert_not_contains "4e showed no approval banner" "policy: ALLOWED" "$RUN_LOG"

echo
echo "==> 4f: the right Safe on the wrong chain."
assert_cmd_fails_with p4f_bad_chain "ChainMismatch" "$BIN" sign --file "$INTENT_BAD_CHAIN"
assert_not_contains "4f showed no approval banner" "policy: ALLOWED" "$RUN_LOG"

assert_taps "the whole of phase 4 cost one sheet: the five policy refusals cost no biometric" \
  "1" "$BIO_PHASE"

if [[ -n "$SKIP_SERVE" ]]; then
  phase "PHASE 5 — SKIPPED (SKIP_SERVE is set)"
  echo "The live daemon, TLS pinning and /read path were NOT exercised."
else
  phase "PHASE 5 — serve and one live /read (1 Touch ID, two terminals)"

  echo "The daemon is NEVER backgrounded by this script. You run it, you stop it."
  echo
  echo "  1. Open a SECOND terminal."
  echo "  2. Paste EXACTLY this and leave it running in the foreground:"
  echo
  echo "     HOT_CHEESE_HOME=\"$HOT_CHEESE_HOME\" NO_COLOR=1 \"$BIN\" serve"
  echo
  ...
  run_cmd p5_health curl -sS --cacert "$HOT_CHEESE_HOME/ssl-cert.pem" \
  ...
  echo
  echo "==> one real df-share read through the pinning client. ONE Touch ID sheet, raised by"
  echo "    the daemon in terminal B. The key is encrypted end-to-end to the client process:"
  echo "    the client prints only its length and digest, never the bytes."
  TAPS_EXPECTED=$((TAPS_EXPECTED + 1))
  BIO_START="$(date '+%Y-%m-%d %H:%M:%S')"
  run_cmd p5_pin_cert cargo run --release --example pin_cert -- \
    "https://127.0.0.1:$DRY_PORT" "$EVM_KEY"
  ...
  assert_eq "the live daemon serves the same key as the CLI" "$ADDR_SE" "$(lower "$PIN_ADDR")"
  ...
  assert_taps "one live /read costs exactly one biometric" "1" "$BIO_START"
...
phase "PHASE 6 — Secure-Enclave loss and the recovery backstop (0 Touch ID, 1 passphrase)"
...
BIO_START="$(date '+%Y-%m-%d %H:%M:%S')"
assert_cmd_fails_with p6_no_se "SeKeyUnavailableTryUnlockPassphrase" "$BIN" address evm "$EVM_KEY"
...
run_tty p6_pass_unlock "$BIN" --unlock passphrase address evm "$EVM_KEY"
...
if [[ -n "$RUN_MIGRATE" ]]; then
  ...
  echo "own test fixture. It does not touch any real legacy store."
  echo
  echo "IT WILL WRITE ONE ITEM TO YOUR LOGIN KEYCHAIN:"
  echo "    service = $MIG_SERVICE"
  echo "    account = $MIG_ACCOUNT"
  echo "    secret  = the PUBLIC test-fixture password from test-keys/key-scrypt.json"
  ...
  if [[ "$MIG_SERVICE" == "$CFG_SERVICE" || "$MIG_ACCOUNT" == "$CFG_ACCOUNT" ]]; then
    ...
    fail "legacy fixture missing: $LEGACY_FIXTURE"
    ...
    write_config "$MIG_SERVICE" "$MIG_ACCOUNT"
    run_cmd pm_migrate "$BIN" migrate --old-store "$MIG_OLD" --new-store "$MIG_NEW"
    assert_eq "migrate exit code" "0" "$RUN_RC"
    assert_contains "migrate reported the fixture key" "name=$LEGACY_KEY_NAME" "$RUN_LOG"
...
phase "PHASE 7 — teardown (0 prompts)"
...
REAL_BLOBS="$(find "$REAL_HOME" -maxdepth 1 -name 'se_kek_*.blob' 2>/dev/null || true)"
assert_eq "no se_kek_*.blob in the real home" "" "$REAL_BLOBS"
...
if security find-generic-password -s "$SE_LABEL" > /dev/null 2>&1; then
  fail "a login-keychain generic password exists with service $SE_LABEL"
else
  pass "no login-keychain generic password with service $SE_LABEL"
fi
if security find-generic-password -l "$SE_LABEL" > /dev/null 2>&1; then
  fail "a login-keychain generic password exists with label $SE_LABEL"
else
  pass "no login-keychain generic password with label $SE_LABEL"
fi
...
if [[ "$FAIL_COUNT" -eq 0 ]]; then
  echo "  DRY RUN CLEAN. The enclave path, the envelope, the policy gate, the daemon and the"
  echo "  teardown all behaved. Proceed with MIGRATION.md against the real home."
  exit 0
fi
