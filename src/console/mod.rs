...
//! [`crate::mac::local_auth::LaContext`] wraps a raw ObjC pointer and is `!Send`, so Touch ID,
...
//! shuttle ciphertext across [`crate::server::PrivilegedOp`] and never unlock anything.
//!
...
use crate::server::{self, Approval, BackendImpl, HotApi, PrivilegedOp, PENDING_OPS};
...
fn session(
    ...
    let log = status::install_subscriber(&home_dir(), crate::cli::env_log_level())?;
    ...
    let serving = match gate {
        ...
        UnlockGate::Biometric => {
            ...
            runtime.spawn(async move {
                ...
                if let Err(e) = server::serve_loop(listener, tls, served_config, approval, rx).await
                {
