...
impl LaContext {
    ...
    pub fn evaluate_biometric(reason: &str) -> Result<Self, SeErr> {
        ...
        unsafe {
            let _: () = msg_send![*obj, setTouchIDAuthenticationAllowableReuseDuration: TOUCH_ID_REUSE_SECS];
        }
        let reason = CString::new(reason).map_err(|_| SeErr::TouchIdDenied)?;
        let localized_reason: *mut Object =
            unsafe { msg_send![class!(NSString), stringWithUTF8String: reason.as_ptr()] };
